...
process_map_styled <- function(eventlog
	...
	edges_performance <- function(precedence, metric_type) {
		...
		precedence %>%
			ungroup() %>%
			mutate(time = case_when(flow_time == "inter_start_time" ~ as.double(next_start_time - start_time, units = attr(metric_type, "units")),
									flow_time == "idle_time" ~ as.double(next_start_time - end_time, units = attr(metric_type, "units")))) %>%
			group_by(act, next_act, from_id, to_id) %>%
			summarize(value = metric_type(time), label = paste0(round(metric_type(time),2), " ", attr(metric_type, "units"))) %>%
			na.omit() %>%
			ungroup() %>%
	...
	edges_frequency <- function(precedence, metric_type) {
		precedence %>%
			...
			mutate(label = case_when(metric_type == "relative" ~ round(100*n/sum(n),2),
									 metric_type == "absolute" ~ n)) %>%
			ungroup() %>%
			arrange(act, from_id, next_act, to_id) %>%
			mutate(penwidth = rescale(label, to = c(1,5)))->i
		return(i)
