...
#' @param colOperation The operation to calculate a single value from the from and the to column
#' @export aggregate




columnAggregate <- function(FUN = mean, colomnName = NULL, colOperation = c("mean", "min", "max", "minus", "plus","from","to")) {
    if (is.null(colomnName)) 
        stop("No column name specified")
    else{
        attr(FUN, "colomnName") <- colomnName
        attr(FUN, "perspective") <- "colomAgregate"
        attr(FUN, "colOperation") <- colOperation
    }
	return(FUN)
}
