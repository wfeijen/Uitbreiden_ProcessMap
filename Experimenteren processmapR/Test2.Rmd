...
debugonce(relative)
process_map_styled(gefilteredeEventLog, metric_type = performance("relative") )
```
```{r}
	debugonce(process_map_styled)
	pm<-process_map_styled(gefilteredeEventLog, metric_type = frequency("relative"),render = F )
	pm.edgese<-pm$edges_df
	pm%>%render_graph()
```
...
map <- map %>%
	...
	palette = "RdYlGn")
map%>%render_graph()
get_edge_df(map)$color<-rgb(0, 1, 1, 0.5)
get_edge_df(map)
```
